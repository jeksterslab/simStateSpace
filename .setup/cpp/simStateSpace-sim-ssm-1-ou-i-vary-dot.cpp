...
Rcpp::List SimSSM1OUIVary(const int n, const Rcpp::List& mu0, const Rcpp::List& sigma0_l, const Rcpp::List& mu, const Rcpp::List& phi, const Rcpp::List& sigma_l, const Rcpp::List& nu, const Rcpp::List& lambda, const Rcpp::List& theta_l, const Rcpp::List& gamma_eta, const Rcpp::List& x, const double delta_t, const int time, const int burn_in) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    y.col(0) = nu_temp + (lambda_temp * eta.col(0)) + (theta_l_temp * arma::randn(num_manifest_vars));

    // Step 3.4: Simulate state space model data using a loop
    for (int t = 1; t < total_time; t++) {
      eta.col(t) = alpha_temp + (beta_temp * eta.col(t - 1)) + (psi_l * arma::randn(num_latent_vars)) + (gamma_eta_temp * x_t.col(t));
      y.col(t) = nu_temp + (lambda_temp * eta.col(t)) + (theta_l_temp * arma::randn(num_manifest_vars));
    }
    ...
    out[i] = Rcpp::List::create(Rcpp::Named("id") = id, Rcpp::Named("time") = arma::linspace(0, (time - 1) * delta_t, time), Rcpp::Named("y") = y.t(), Rcpp::Named("eta") = eta.t(), Rcpp::Named("x") = x_t.t());
  }
