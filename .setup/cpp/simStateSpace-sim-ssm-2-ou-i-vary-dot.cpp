...
Rcpp::List SimSSM2OUIVary(const int n, const Rcpp::List& mu0, const Rcpp::List& sigma0_sqrt, const Rcpp::List& mu, const Rcpp::List& phi, const Rcpp::List& sigma_sqrt, const Rcpp::List& nu, const Rcpp::List& lambda, const Rcpp::List& theta_sqrt, const Rcpp::List& gamma_y, const Rcpp::List& gamma_eta, const Rcpp::List& x, const double delta_t, const int time, const int burn_in) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    y.col(0) = nu_temp + lambda_temp * eta.col(0) + theta_sqrt_temp * arma::randn(num_manifest_vars) + gamma_y_temp * x_t.col(0);

    // Step 3.4: Simulate state space model data using a loop
    for (int t = 1; t < total_time; t++) {
      eta.col(t) = alpha_temp + beta_temp * eta.col(t - 1) + psi_sqrt * arma::randn(num_latent_vars) + gamma_eta_temp * x_t.col(t);
      y.col(t) = nu_temp + lambda_temp * eta.col(t) + theta_sqrt_temp * arma::randn(num_manifest_vars) + gamma_y_temp * x_t.col(t);
    }
