...
Rcpp::List SimSSMFixed2(const int n, const int time, const double delta_t,
                        ...
                        const Rcpp::List& x, const arma::mat& gamma_eta, const arma::mat& gamma_y) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    y.col(0) = nu + (lambda * eta.col(0)) + (theta_l * arma::randn(k)) + (gamma_y * x_i.col(0));
    // Step 3.3: Data generation loop
    for (int t = 1; t < time; t++) {
      eta.col(t) = alpha + (beta * eta.col(t - 1)) + (psi_l * arma::randn(p)) + (gamma_eta * x_i.col(t));
      y.col(t) = nu + (lambda * eta.col(t)) + (theta_l * arma::randn(k)) + (gamma_y * x_i.col(t));
    }
