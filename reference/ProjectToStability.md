...
are stationary. If the matrix is already within the margin, it is
returned unchanged.
...
A numeric matrix of the same dimensions as `x`, scaled if necessary to
satisfy the stability constraint.

## Details

The projection is performed by multiplying the matrix by a constant
factor \\c = \frac{\text{margin}}{\rho + \text{tol}}\\, where \\\rho\\
is the spectral radius and `tol` is a small positive number to prevent
division by zero.
