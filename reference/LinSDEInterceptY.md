...
The intercept vector for the observed variables in the linear stochastic
differential equation model \\\boldsymbol{\nu}\\ is given by \$\$
\boldsymbol{\nu} = \mathrm{Mean} \left( \mathbf{y} \right) -
\boldsymbol{\Lambda} \mathrm{Mean} \left( \boldsymbol{\eta} \right) \$\$
where \\\boldsymbol{\Lambda}\\ is the matrix of factor loadings,
\\\mathrm{Mean} \left( \mathbf{y} \right)\\ is the steady-state mean
vector for the observed variables, and \\\mathrm{Mean} \left(
