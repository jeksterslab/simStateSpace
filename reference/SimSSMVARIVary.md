...
a state space model parameterization. It assumes that the parameters can
...
`alpha`, `beta`, `psi_l`, `gamma`, or `kappa`) is less the `n`, the
function will cycle through the available values.
