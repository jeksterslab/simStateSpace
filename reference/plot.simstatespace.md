...
- id:
  ...
  Numeric vector. Optional `id` numbers to plot. If `id = NULL`, plot
  all available data.

- time:

  Numeric vector. Optional `time` points to plot. If `time = NULL`, plot
  all available data.

- eta:

  Logical. If `eta = TRUE`, plot the latent variables. If `eta = FALSE`,
