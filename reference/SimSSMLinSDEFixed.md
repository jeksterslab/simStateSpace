...
This function simulates data from the linear stochastic differential
equation model using a state space model parameterization. It assumes
that the parameters remain constant across individuals and over time.
...
The dynamic structure is given by \$\$ \mathrm{d} \boldsymbol{\eta}\_{i,
t} = \left( \boldsymbol{\iota} + \boldsymbol{\Phi}
\boldsymbol{\eta}\_{i, t} \right) \mathrm{d}t + \boldsymbol{\Gamma}
...
The state space parameters as a function of the linear stochastic
differential equation model parameters are given by \$\$
\boldsymbol{\beta}\_{\Delta t\_{{l\_{i}}}} = \exp{ \left( \Delta t
\boldsymbol{\Phi} \right) } \$\$

\$\$ \boldsymbol{\alpha}\_{\Delta t\_{{l\_{i}}}} =
\boldsymbol{\Phi}^{-1} \left( \boldsymbol{\beta} - \mathbf{I}\_{p}
\right) \boldsymbol{\iota} \$\$
...
where \\p\\ is the number of latent variables and \\\Delta t\\ is the
time interval.
