...
void function_dynam(const double tstart, const double tend, size_t regime, const gsl_vector *xstart,
	...
	switch (regime) {
		case 0:
			gsl_vector_set(x_tend, 0, param[0] * 1 + param[3] * gsl_vector_get(xstart, 0) + param[4] * gsl_vector_get(xstart, 1) + param[5] * gsl_vector_get(xstart, 2));
			gsl_vector_set(x_tend, 1, param[1] * 1 + param[6] * gsl_vector_get(xstart, 0) + param[7] * gsl_vector_get(xstart, 1) + param[8] * gsl_vector_get(xstart, 2));
			gsl_vector_set(x_tend, 2, param[2] * 1 + param[9] * gsl_vector_get(xstart, 0) + param[10] * gsl_vector_get(xstart, 1) + param[11] * gsl_vector_get(xstart, 2));
			break;
	}
}
...
void function_jacob_dynam(const double tstart, const double tend, size_t regime, const gsl_vector *xstart,
	...
	switch (regime) {
		case 0:
			...
			gsl_matrix_set(Jx, 0, 1, param[4]);
			gsl_matrix_set(Jx, 0, 2, param[5]);
			gsl_matrix_set(Jx, 1, 0, param[6]);
			gsl_matrix_set(Jx, 1, 1, param[7]);
			gsl_matrix_set(Jx, 1, 2, param[8]);
			gsl_matrix_set(Jx, 2, 0, param[9]);
			gsl_matrix_set(Jx, 2, 1, param[10]);
			gsl_matrix_set(Jx, 2, 2, param[11]);
			break;
	}
}
